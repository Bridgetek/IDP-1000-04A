...
#define PDN_OUT(PDN_PIN)
#define PDN_HIGH(PDN_PIN)			gpio_put(PDN_PIN ,1)
#define PDN_LOW(PDN_PIN)			gpio_put(PDN_PIN ,0)
...
void setSPI(EVE_HalContext *phost, EVE_SPI_CHANNELS_T numchnls, uint8_t numdummy)
{
	...
	//PDN_OUT(FT8xx_PD);
	PDN_LOW(FT8xx_PD);
	sleep_ms(POWER_DOWN_CYCLE_DELAY);
	PDN_HIGH(FT8xx_PD);
	sleep_ms(POWER_DOWN_CYCLE_DELAY);

	EVE_Hal_hostCommand(phost,FT8xx_EXOSC);
	EVE_Hal_hostCommand(phost,FT8xx_CLOCKSEL | (0x46 << 8));
	sleep_ms(100);
	EVE_Hal_hostCommand(phost,FT8xx_ACTIVE);
	sleep_ms(200);
